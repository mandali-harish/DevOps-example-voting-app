locals {
  dualstack_oidc_issuer_url = try(replace(replace(aws_eks_cluster.this.identity[0].oidc[0].issuer, "https://oidc.eks.", "https://oidc-eks."), ".amazonaws.com/", ".api.aws/"), null)
}
...
output "cluster_service_cidr" {
  ...
  value       = var.cluster_ip_family == "ipv6" ? try(aws_eks_cluster.this.kubernetes_network_config[0].service_ipv6_cidr, null) : try(aws_eks_cluster.this.kubernetes_network_config[0].service_ipv4_cidr, null)
}
